...
void GameLog::LogMonsterSpawn(Monster& MonsterRef) // Monster 클래스 GetName()이 const 함수가 아니라 const 삭제했습니다!
{
    ...
    cout << "<LOG>\t[" << name << "] 등장 (누적 스폰: " << MonsterSpawnCount[name] << "회)" << endl;
}
...
void GameLog::LogPlayerAttack(Character* player, Monster* monster) // 플레이어가 공격했을 때
{
    // 전체 누적 데이터
    CharacterAttackCount++;       // 총 공격 횟수
    TotalDealtDamage += player->GetPower();   // 누적 가한 데미지

    // 배틀 화면용 카운터 누적
    CurrentBattleAttackCount++;
    CurrentBattleDealtDamage += player->GetPower();

    // 지정 포맷으로 실시간 배출
    cout << "<LOG>\t" << monster->GetName() << " 조우 | 현 전투 공격: " << CurrentBattleAttackCount
        << "회 | 현 전투 가한 데미지: " << CurrentBattleDealtDamage << endl;
}

// 플레이어가 피격당했을 때
void GameLog::LogPlayerDamaged(Monster* monster)
{
    // 전체 누적 데이터
    TotalReceivedDamage += monster->GetPower(); // 누적 입은 피해량

    // 배틀 화면용 카운터 누적
    CurrentBattleReceiveCount++;
    CurrentBattleReceiveDamage += monster->GetPower();

    // 지정 포맷으로 실시간 배출
    cout << "<LOG>\t" << monster->GetName() << " 조우 | 현 전투 피격: " << CurrentBattleReceiveCount
        << "회 | 현 전투 입은 피해량: " << CurrentBattleReceiveDamage << endl;
}
...
void GameLog::LogItemUse(Item& ItemRef) //const 삭제했습니다!
{
    ...
    cout << "<LOG>\t" << "[아이템] " << name << " 사용 (누적 사용: " << ItemUseCount[name] << "회)" << endl;
}
...
void GameLog::LogMonsterKill(string MonsterName)
{
    ...
    cout << "<LOG>\t" << MonsterName << " 처치 | 누적 처치 수: " << MonsterKills[MonsterName] << "마리" << endl;
}
...
{
    cout << "<LOG>\t" << "[이동] -> " << Location << endl; // 즉시 화면 배출
